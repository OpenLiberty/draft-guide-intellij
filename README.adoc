...
Learn how to use IntelliJ IDE to develop microservice with Open Liberty..

== What you'll learn

You will learn how to get started with IntelliJ, an integrated development environment written in Java for developing computer software，by building and testing a simple REST service which will expose the JVM's system properties. The REST service will respond to `GET` requests made to the
...
- *Intellij:* IntelliJ IDEA is an integrated development environment written in Java for developing computer software. For installation instructions, refer to the official https://www.jetbrains.com/idea/download/[Intellij download documentation^].

Learning basic setup and features in https://www.jetbrains.com/idea/documentation/[Intellij official documentation^].
...
For the `Name` and in `Command line` field, put `liberty:stop`. In the `Name` field, put `Stop finish`.

Finally, click the `+` button at the top-left in the `Run/Debug Configurations` pop up window. Select `Maven`
in `Add new configuration`.

For the `Name` and in `Command line` field, put `liberty:dev`. In the `Name` field, put `Develop start`.
