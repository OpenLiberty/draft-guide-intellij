...
To run the test, click the green `Run` button in the right panel of `Liberty Dev Dashboard`. Double click `Run Tests` in the drop down menu. You will see the following output:
...
When you are done checking out the service, exit development mode by expanding the `Liberty Dev Dashboard` in the right panel and click the green `Run` button. Select `guide-intellij` in the pop up window and click `OK`. After that, select `Start` in the drop-down list and start the dev mode.
