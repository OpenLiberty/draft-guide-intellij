...
- *JDK:* Install https://adoptopenjdk.net/?variant=openjdk8&jvmVariant=openj9/[Java 8 JDK^]. The version of JDK is `OpenJDK 8 (LTS)` and JVM is `OpenJ9`.
...
In the `Project SDK` section, click `New`, choose `JDK` and specify the path to the `Java 8 JDK` downloaded on your computer.
...
You are expected to see `This site can't be reached`.

== Configuring the server

On the top left hand side of the screen, expand the `guide-{projectid}` project and navigate to the `start` directory to begin.
