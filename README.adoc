...
Next, repeact the above steps using the following values in the corresponding fields:
...
One last time, using the following values in the corresponding fields:
...
can be written to call the application server directly. In this example, you will run a test that
does the latter.
...
Since you started Open Liberty in development mode earlier in the guide, at the bottom left of the tool bar, navigate to the `Run` window and select the tab for `Develop start`. Press `enter/return` key to run the tests. You will see the following output:
...
To see whether the tests detect a failure, add an [hotspot=assertequals file=0]`assertion` that you know fails, or change the existing [hotspot=assertosname file=0]`assertion` to a constant value that doesn't match the `os.name` system property.
