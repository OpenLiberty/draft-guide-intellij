...
From the main menu, select `Run` and click `Edit Configurations`. Click the `+` button at the top-left in the `Run/Debug Configurations` pop up window. Select `Maven` in `Add new configuration`.
...
Click the `+` button at the top-left in the `Run/Debug Configurations` pop up window. Select `Maven` in `Add new configuration`.
...
To try out the application, you can select `Run` in the main menu and click `Run...`. Select `liberty:run` in the pop up window and deploy it to Open Liberty.

Check out the service at the
http://localhost:9080/LibertyProject/System/properties[^] URL.

After you are done checking out the application, stop the Open Liberty server by pressing `CTRL+C` in the shell session where you ran the server. Alternatively, you can select `Run` in the main menu and click `Run...`. Select `liberty:stop` in the pop up window.

== Creating a JAX-RS application

Navigate to the `start` directory to begin.

you can select `Run` in the main menu and click `Run...`. Select `liberty:dev` in the pop up window and start Open Liberty in development mode, which starts the Open Liberty server and listens for file changes.
