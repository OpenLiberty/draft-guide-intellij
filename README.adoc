...
After you are done checking out the application, stop the Open Liberty server by pressing `CTRL+C` in the shell session where you ran the server. Alternatively, you can select `Run` in the menu bar and click `Run...`. Select `liberty:stop` in the pop up window.

== Creating a JAX-RS application

Navigate to the `start` directory to begin.

you can select `Run` in the menu bar and click `Run...`. Select `liberty:dev` in the pop up window and start Open Liberty in development mode, which starts the Open Liberty server and listens for file changes.
