...
Start IntelliJ IDEA, select `Import Project`. Navigate to the directory that the project was saved and click `Open`. On the `Import Project` dialog box, choose `Import project from external model` and select `Maven` and click `Finish`.

If the directory is not automatically open, click `Open` on the welcome screen and navigate to the directory mentioned previously and select `Open`. Select `View` in the menu bar and navigate to `Tool Windows` and click `Project` to display the `Project Tool Window` at top left side. Choose `Project Files` in the dropdown menu and expand the `guide-{projectid}` project. 
